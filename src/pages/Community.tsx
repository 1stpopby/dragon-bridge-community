...
const Community = () => {
  ...
  const fetchGroups = async () => {
    try {
      ...
      toast({
        title: "Eroare la încărcarea grupurilor",
        description: "Nu s-au putut încărca grupurile comunitare din baza de date.",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-8">
            ...
            <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
              Conectează-te cu comunitățile românești din UE. Găsește grupul tău local, 
              cunoaște oameni cu aceleași interese și construiește prietenii durabile.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 max-w-4xl mx-auto mb-8">
            <div className="text-center">
              ...
              <div className="text-sm text-muted-foreground">Total Membri</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-primary">{totalGroups}</div>
              <div className="text-sm text-muted-foreground">Grupuri Locale</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-primary">{eventsThisMonth}</div>
              <div className="text-sm text-muted-foreground">Evenimente Luna Aceasta</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-bold text-primary">{uniqueLocations}</div>
              <div className="text-sm text-muted-foreground">Orașe Acoperite</div>
            </div>
          </div>
        ...
        <Tabs defaultValue="groups" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-8">
            <TabsTrigger value="groups">
              Grupuri Comunitare ({filteredGroups.length})
            </TabsTrigger>
            <TabsTrigger value="leaders">Lideri Remarcabili</TabsTrigger>
          </TabsList>

          <TabsContent value="groups">
            {loading ? (
              <div className="text-center py-12">
                <p className="text-muted-foreground">Se încarcă grupurile...</p>
              </div>
